...
export function TelemetryCards() {
  ...
  return <div className="mt-[var(--space-2)] grid grid-cols-2 gap-[var(--space-1)] sm:grid-cols-4">
    <TelemetryCard label="Tracked objects" value={unavailable ? "--" : undefined}><AnimatedNumber value={data?.trackedObjects} /></TelemetryCard>
    <TelemetryCard label="Close approaches" tone="accent" value={unavailable ? "--" : undefined}><AnimatedNumber value={data?.closeApproaches} /></TelemetryCard>
    <TelemetryCard label="Hazard index" tone="warning" value={unavailable ? "--" : undefined}><AnimatedNumber value={data?.hazardIndex} /></TelemetryCard>
    <TelemetryCard label="Data latency" unit="ms" value={unavailable ? "--" : undefined}><AnimatedNumber value={data?.latencyMs} /></TelemetryCard>
  </div>;
}
